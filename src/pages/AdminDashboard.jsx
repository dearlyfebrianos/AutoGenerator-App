...
const AdminDashboard = () => {
  ...
  const handleMediaChange = (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toast.error("Ukuran media maksimal 2MB untuk pengumuman!");
      return;
    }
    ...
    if (!validTypes.includes(file.type)) {
      toast.error(
        "Format tidak didukung! Gunakan JPG, PNG, GIF, WebP, MP4, atau WebM",
      );
      return;
    }
  ...
  const handlePublish = () => {
    ...
    const shouldIncludeMedia =
      announceMedia && announceMedia.size <= 2 * 1024 * 1024;

    const announceData = {
      text: announceText.trim(),
      media: shouldIncludeMedia ? mediaPreview : null,
      mediaType: shouldIncludeMedia ? mediaType : null,
    ...
    try {
      ...
      if (error.name === "QuotaExceededError") {
        toast.error(
          "Gagal menyimpan: file media terlalu besar. Gunakan gambar/video < 2MB atau hapus media.",
          { duration: 5000 },
        );
      } else {
        toast.error("Gagal menyimpan pengumuman.", { duration: 3000 });
      }
    }
  };
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {announceText.length}/10.000 karakter
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 border border-purple-200 dark:border-purple-700 rounded-lg p-4">
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 mt-3">
                    Dipublikasikan:{" "}
                    {new Date(currentAnnounce.publishedAt).toLocaleString(
                      "id-ID",
                    )}
                  </p>
                </div>
